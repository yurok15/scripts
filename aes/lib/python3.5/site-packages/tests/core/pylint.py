...
def pytest_collect_file(path, parent):
    ...
    if not config.option.aespylint:
        return
    if path.ext != ".py":
        return

    # Find pylintrc to check ignore list
    pylintrc_file = config.option.aespylint_rcfile or PYLINTRC
...
class PyLintItem(pytest.Item, pytest.File):
    ...
    def runtest(self):
        ...
        for error in reporter.data:
            if error.C in self.config.option.aespylint_error_types:
